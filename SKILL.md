---
name: xxd-panel-020
description: "Create XXD Panel 020 artwork from supplied photos in four combinable modes: photo above/minimal abstract-memory editorial below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Deconstructs, filters, distils, and reconstructs source identity through a few geometric forms that preserve distinctive mass ratios, axes, curves, interlocks, layers, occlusion, negative space, and asymmetry, using a strict source-derived palette and art-book microtypography. Use for the exact 020 modern quiet collectible abstraction; never use it for generic icons, meaningless geometry, evenly distributed shapes, fragmented collage, commercial templates, or multi-photo collage."
---

# XXD Panel 020 · 极简抽象记忆编辑

Turn each supplied photograph into one finished minimal abstract-memory editorial work. Preserve the real photograph only in paired modes; every transformed frame must pass through deconstruct → select → distil → reconstruct, retaining the source's most distinctive mass ratio, axis, curve, interlock, layering, occlusion, negative space, and asymmetry in only a few geometric forms. The result must correspond to the source at a glance without becoming a literal outline, generic icon, or meaningless abstraction.
...
- The transformed frame must preserve source identity through at least three specific structural cues across mass ratio, axis, curve, interlock, layer, occlusion, negative shape, scale difference, or asymmetry. Use only the few geometric forms necessary to carry one source-bound visual memory. It is not literal tracing, a generic icon, evenly spaced diagram, decorative shape set, fragmented collage, stock Bauhaus composition, commercial template, photoreal painting, or unrelated abstraction.
...
Modes and device ratios may change placement and aspect ratio but never the 020 motive. Every transformed frame must visibly express this chain:

**this exact photographed subject or inseparable relation → preserve at least three distinctive cues across mass proportion, axis, curve, interlock, layering, occlusion, negative space, scale difference, and asymmetry → deconstruct the photograph into structural facts → select only the facts essential to recognition and memory → distil them into a few geometric forms → reconstruct one visual centre through hierarchy, scale contrast, positive/negative form, and breathing space → derive exactly one main colour, one dark structural colour, one light or neutral colour, and at most a small key accent from the source → integrate one poetic 2–5-word title plus two to four microtext groups along axes, edges, contours, colour blocks, and negative spaces → make geometry carry visual memory and type carry order, rhythm, and editorial intelligence**.

The abstraction comes from the source's actual volume, axis, curve, opening, overlap, obstruction, direction, distance, and asymmetric balance—not from attaching fashionable circles, arches, bars, or modernist grids. Artist books, architectural journals, exhibition catalogues, minimal abstraction, and precise editorial systems are construction influences rather than templates.

Reject the result as generic when an unrelated photo could replace the source without materially changing the mass ratios, axes, curves, interlocks, occlusions, negative shapes, palette proportions, or copy path. Also reject complete literal reconstruction, iconification, average alignment, scattered fragments, too many shapes, evenly distributed colours, grey overall grading, meaningless abstraction, cheap ornament, complex collage, and template repetition.

## 020 visual system

### Deconstruct, select, distil, reconstruct

- Privately lock at least three source-specific structural cues across mass proportion, axis, curve, opening, connection, interlock, layering, occlusion, negative shape, scale difference, distance, or asymmetry.
- Deconstruct the source into those facts; select only what is essential to recognition; distil each selected fact into one geometric role; reconstruct a coherent memory rather than a diagram of all parts.
- People retain pose, direction, body/clothing mass, and relational distance; animals retain body rhythm and head direction; plants retain growth curve; architecture retains skyline, axes, and defining openings; objects and vehicles retain functional mass and negative shape; landscapes retain a source-specific horizon, terrain curve, or spatial relation.
- Preserve correspondence at a glance without completing every contour. Every form must carry identity, weight, action, spatial relation, or memory.

### Few forms, one visual memory

- Use a small, disciplined form vocabulary. One dominant relationship and one visual centre are enough; secondary forms only clarify scale, axis, occlusion, or negative space.
- Compose through visual centre of gravity, scale difference, primary/secondary hierarchy, positive/negative form, overlap, alignment, and breathing rhythm. The result may be offset, cropped, extended, or suspended when the source supports it.
- Preserve a clean ivory ground and generous whitespace. Do not evenly distribute elements, align everything to a grid, fill empty areas, or fragment the subject into decorative debris.
- Recompose the few essential forms for every aspect ratio. Keep mass relation, asymmetry, key negative space, title, and reading path intact rather than cropping another output.

### Source-bound geometry

- Convert source mass into circles, ellipses, rectangles, bars, arcs, wedges, irregular planes, or custom simple contours only when each choice follows a locked structural cue.
- Preserve the source's distinctive ratios and relationships: a long axis versus compact mass, curve against bar, opening inside body, one form passing behind another, a blocked edge, a narrow gap, or a deliberate imbalance.
- Allow flat overlap and restrained depth cues, but do not create photoreal volume, 3D objects, gradients, or decorative pseudo-geometry.
- Reject icon silhouettes, logo marks, evenly sized modules, repeated motifs, arbitrary circles, stock constructivist arrangements, and shapes that could survive unchanged on an unrelated source.

### Strict source-derived colour roles

- Extract and assign one main colour, one dark structural colour, and one light or neutral colour from the source. Add at most one small key accent only when the source clearly supports it.
- Preserve the source's most recognisable and spirited colour rather than averaging pixels or greying the palette. Use area proportion, value difference, and local contrast to build hierarchy.
- Keep the ivory background visibly separate from the light/neutral form. The main colour carries memory; the dark colour anchors axes, overlap, or type; the light/neutral colour opens space.
- Reject equal colour shares, indiscriminate multicolour, overall desaturation, fixed palettes, gradients, atmospheric shading, muddy ageing, and decorative colour unrelated to the source.

## Copy and typography belong to 020

020 text is a complete art-book microtypographic system that gives the abstraction structure, rhythm, and reading order—not a detached headline, generic caption, UI overlay, or pseudo-intellectual filler.

- Automatic copy derives one poetic 2–5-word title from light, space, action, material, relation, or emotion, plus two to four microtext groups chosen from a short phrase, state word, supplied place or object information, index, chapter mark, sequence code, scale-like number, direction word, material word, archival label, or micro-explanation.
- Automatic copy never invents or uses a year. Dates, places, provenance, and factual numbers must be supplied or reliably established. Exact user wording remains verbatim even when it contains a year; the no-year rule constrains automatic invention, not user copy.
...
Typography has its own 020 construction:

- **Role:** the title carries emotion; microtype carries order, evidence, material, sequence, and reading rhythm.
- **Voice:** poetic, quiet, precise, architectural, observant, and source-specific.
- **Construction:** one 2–5-word title plus two to four microtext groups, with deliberate scale contrast between at least one very small and one slightly larger group.
- **Placement:** align along geometric axes, edges, contours, colour blocks, or negative space; use vertical setting, rotation, columns, tracking, edge attachment, corner pressure, crossing, nesting, or alignment only when script-appropriate and structurally useful.
- **Material:** restrained serif plus slender editorial sans equivalents, crisp and flat within the abstract system; no glow, extrusion, sticker effect, or generic bold display face.
- **Script:** use native editorial serif and slender-text equivalents for the resolved locale. Preserve natural Chinese, Japanese, Korean, Arabic, and Latin spacing, shaping, orientation, and semantic line breaks rather than forcing Latin tracking or rotation onto every script.
...
Prefer GPT Image 2 through the host's built-in bitmap image-generation capability when exposed, and follow the available `imagegen` skill for the built-in execution details. View each local source before generation. Use one generation call per distinct asset; a wallpaper pack requires four calls. “Minimal abstraction”, “geometric editorial”, “abstract memory”, and “art-book microtype” describe appearance only and never authorise SVG, HTML, CSS, Canvas, diagrams, or programmatic drawing as the artwork.
...
Use only images attached to the current invocation, explicit user paths, or a previously supplied image that the current request clearly identifies as “the same image”. Never broadly scan the Desktop, workspace, `~/Desktop/xxd/xxd-panel-020/`, or historical task folders to find a substitute source. If the intended source cannot be accessed, ask for it.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，020 设计在下）
   2. 左右对照（原图在左，020 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-020-prompt.en.md` or `references/xxd-panel-020-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 020 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 020 transformation on the right.
   - `design-only`: generate the 020 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 020 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 020-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 020-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
`scripts/compose_panel.py` remains available for deterministic recovery and audit. Trigger it only under step 11. When used, generate a same-aspect design asset from the full 020 prompt, preserve the source without destructive crop or stretch, and document which fallback condition applied. A direct complete-canvas success must not be split and recomposed again.
...
Save finished work under `~/Desktop/xxd/xxd-panel-020/` unless the user supplies another destination. Create the shared `~/Desktop/xxd/` wrapper, the skill root, and each task directory when needed.
...
- A few geometric forms preserve at least three source-specific structural cues across mass ratio, axis, curve, interlock, layer, occlusion, negative shape, scale difference, or asymmetry.
- The abstraction visibly follows deconstruct → select → distil → reconstruct and corresponds to the source at a glance without literal reconstruction or iconification.
- One visual centre, strong scale hierarchy, deliberate positive/negative form, and generous ivory whitespace replace even distribution, average alignment, fragmented debris, or excessive shape count.
- Colour roles are strict and source-derived: one main colour, one dark structural colour, one light or neutral colour, and at most a small key accent. Area ratios, value contrast, and local contrast establish hierarchy without overall greying.
- There is no generic icon, logo, stock Bauhaus layout, meaningless circle-and-bar arrangement, gradient, 3D volume, complex collage, cheap ornament, commercial template, or unrelated abstraction.
- Automatic copy is concise, grounded, and 020-specific; exact user wording remains verbatim. All rendered text is accurate and native to the resolved locale; text-free output contains no text or pseudo-text.
- Typography forms one coherent editorial system: a poetic 2–5-word title and two to four accurate microtext groups create a deliberate reading path along axes, edges, contours, colour blocks, and negative space. Automatic copy contains no invented year; exact user wording remains verbatim.
...
User instructions may change subject emphasis, selected structural cues, form vocabulary, mass ratio, positive/negative balance, colour roles, type path, title voice, or microtext mix within 020, but do not silently relax one-photo isolation, exact paired geometry, four separate wallpaper outputs, fresh-task generation, source-hidden output rules, native-language typography, few-form discipline, or source-derived palette roles. Leave the minimal abstract-memory editorial aesthetic only when the user explicitly asks to leave the 020 style.

## Provenance boundary

The user's original style brief is archived at [references/020-source.md](references/020-source.md). It records the initial 3:4 top-bottom example but does not override the operative mode and explicit final-canvas selection and complete-canvas generation rules. The full local 020 prompt is the production specification. Never borrow subjects, colours, copy, or compositions from samples or previous outputs.
