<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 020 프로젝트 배너" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-추상-기억-논리)
...
그 답답함 때문에 **Panel 020**을 만들었습니다. 템플릿을 덮는 대신 사진에서 절대 잃으면 안 될 것을 먼저 찾고, **사진을 미니멀 추상 기억과 완전한 편집 체계로 정제합니다**로 번역합니다.
...
> XXD Panel 020으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090148356597428723"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 020 예시 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090148356597428723"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 020 예시 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090148356597428723"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 020 예시 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090148356597428723"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 020 예시 4"></a></td>
  </tr>
  <tr>
    <td colspan="2"><img src="./assets/examples/sample-05.png" alt="XXD Panel 020 예시 5"></td>
  </tr>
  <tr>
    <td width="33%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 020 예시 6"></td>
    <td width="33%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 020 예시 7"></td>
    <td width="33%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 020 예시 8"></td>
  </tr>
</table>
...
이 예시는 020의 미학적 의도를 보여 줄 뿐이며, 예시의 주제, 구성, 색상, 문구, 이전 캔버스 비율은 생성 참고나 현재 기본값이 되지 않습니다.

> 해체—선별—정제—재구성 · 소수 형태 · 엄격한 색 역할 · 아이보리 여백 · 아트북 미세 조판

XXD Panel 020는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진을 해체하고 선별하고 정제한 뒤 재구성하여, 가장 고유한 질량 비율, 축, 곡선, 교차, 겹침, 가림, 음의 공간, 크기 차이와 비대칭만 소수의 기하 형태로 남기고 원본에 한눈에 대응하는 추상 기억을 만듭니다.

색은 원본에서만 가져와 주색 하나, 어두운 구조색 하나, 밝은색 또는 중성색 하나와 필요한 경우 작은 강조색으로 제한합니다. 2–5단어의 시적 제목은 감정을, 2–4개 미세 문자 그룹은 질서, 근거, 재질과 읽기 리듬을 담당합니다. 기하는 시각 기억을, 문자는 편집 경로를 만듭니다.
...
## 빠른 판단: XXD Panel 020이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 미니멀 추상 기억과 완전한 편집 체계로 정제합니다 |
| **한눈에 알아볼 특징** | 해체—선별—정제—재구성 · 소수 형태 · 엄격한 색 역할 · 아이보리 여백 · 아트북 미세 조판 |
...
일반적인 ‘미니멀 기하’는 범용 아이콘, 임의의 원과 막대, 균등 배열, 기성 바우하우스 템플릿 또는 어떤 사진에도 붙는 장식 형태로 쉽게 무너집니다.

020은 순서를 뒤집습니다.

```text
질량／축／곡선／교차／가림／음의 공간／비대칭 고정 → 원본 구조 해체 → 인식에 필요한 소수 관계 선별 → 명확한 기하 역할로 정제 → 강한 크기 위계의 한 중심으로 재구성 → 주색／어두운 구조색／밝은·중성색 배정 → 제목과 2–4개 미세 문자로 읽기 경로 구축
```

무관한 사진으로 바꿔도 질량 비율, 축, 곡선, 교차, 음의 형태, 색 면적과 문자 경로가 거의 그대로 성립한다면 020가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **원본 구조:** 최소 세 가지 고유 단서가 질량 비율, 축, 곡선, 교차, 겹침, 가림, 음의 공간, 크기 차이, 비대칭에서 옵니다.
- **네 단계:** 구조를 해체하고 필요한 관계를 선별하고 기하 역할로 정제한 뒤 추상 기억으로 재구성합니다.
- **소수 형태:** 원본에 근거한 원, 타원, 사각형, 막대, 호, 쐐기, 불규칙 면 또는 단순 고유 윤곽만 씁니다.
- **하나의 시각 기억:** 강한 크기 차이, 위계, 양·음 형태, 가림, 넓은 아이보리 여백으로 하나의 중심을 만듭니다.
- **엄격한 색 역할:** 주색 하나, 어두운 구조색 하나, 밝은색 또는 중성색 하나와 필요한 경우 작은 강조색만 씁니다.
- **완전한 문자 체계:** 2–5단어 제목과 2–4개 미세 문자 그룹이 축, 가장자리, 윤곽, 색 면, 음의 공간을 따라 읽기 경로를 만듭니다.
- **원본 프롬프트 생성 문구에 연도 없음:** 자동 창작은 연도를 만들거나 쓰지 않으며 사용자 확정 문구에 연도가 있어도 그대로 보존합니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 020의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
```text
/xxd-panel-020 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-020 photo.jpg --prefs last
/xxd-panel-020 photo.jpg --prefs edit
/xxd-panel-020 photo.jpg --prefs new
```
...
/xxd-panel-020 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-020 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-020 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR

# 연결형 4종 배경화면: 기준 이미지 후 기기별 재구성
/xxd-panel-020 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# 독립형 배경화면: 기기별 정확한 해상도 지정
/xxd-panel-020 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# 원본 이미지 비율 따르기
/xxd-panel-020 photo.jpg --mode design-only --size source --text none

# 정사각형·세로·가로 완성 구도를 각각 생성
/xxd-panel-020 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# 사용자 비율과 정확한 픽셀을 함께 지정
/xxd-panel-020 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# 원본 프롬프트가 문구를 만들고 언어만 제한
/xxd-panel-020 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-020 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-020 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 020 적용, 공통 설정은 한 번만 확정
/xxd-panel-020 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR

# 반복 매개변수도 누적하며 지정 루트 아래로 출력
/xxd-panel-020 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-020`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-020-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-020-prompt.en.md)
...
**추상은 사진을 지우는 일이 아니라, 바꿀 수 없는 관계만 남기는 일입니다.**
