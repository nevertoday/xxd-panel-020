...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#조합-가능한-네-가지-출력)
...
**Panel 020**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 실제 임파스토에 박힌 촉각적 입체 미니어처 장면으로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
[언어 디렉터리 열기](references/original-prompt/): [중국어 교정본](references/original-prompt/zh-CN.md) · [English](references/original-prompt/en.md) · [日本語](references/original-prompt/ja.md) · [한국어](references/original-prompt/ko.md) · [العربية](references/original-prompt/ar.md)

020 게시물 본문은 “임파스토 × 입체감” 제목과 네 장의 예시와 달리 015의 기하 추상 프롬프트를 중복했습니다. [출처 기록](references/original-prompt/source-tweet-body-2026-08-19.md)이 이 충돌을 남깁니다. 네 원본 예시에서 재구성한 중국어 교정본만 실행 시 권위이며 다른 언어는 충실한 열람 번역입니다.
...
| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 원본 장면을 임파스토에 박힌 입체 미니어처로 재구성 |
| **한눈에 알아볼 특징** | 임파스토 섬 · 입체 미니어처 · 실제 나이프 자국 · 넓은 종이 여백 · 절제된 편집 글자 |
...
흔한 임파스토 스타일은 장식성 튀김, 플라스틱 3D 장난감, 또는 사각 프레임에 붙인 축소 장면이 되기 쉽습니다. 020은 안료, 미니어처 주체, 색과 문구를 원본 사진에 결속합니다.

```text
주체／행동／관계／감정 식별 → 무관한 배경 제거 → 원본의 강한 색 관계 추출 → 물감 섬의 방향과 크기 결정 → 주체를 안료에 박힌 입체로 조형 → 축적／긁힘／끊어진 가장자리／두께 그림자 표현 → 종이 여백에 원본과 결속된 문구만 배치
```

무관한 사진으로 바꿔도 물감 섬, 미니어처 주체, 색 관계와 문구가 그대로 성립한다면 020이 아닙니다.

## 완성본에서 식별되는 특징

- 주체, 행동과 핵심 관계가 한눈에 알아보여야 합니다.
- 방향성 있는 임파스토 섬 하나가 유일한 시각 앵커가 됩니다.
- 물감 축적, 능선, 끊어진 가장자리, 드러난 바탕, 광택과 두께 그림자가 보입니다.
- 미니어처는 안료에서 자라고 가라앉고 겹치거나 지지되며 사각 프레임에 붙지 않습니다.
- 주도 색역은 원본의 의미 있는 색 관계에서 나오며 고정 팔레트를 쓰지 않습니다.
- 거친 따뜻한 백색 또는 아이보리 종이의 넓은 여백을 유지합니다.
- 문구는 사진과 강하게 연결하며 사용자의 정확한 문구는 그대로 사용합니다.

<details>
<summary><strong>전체 기능과 매개변수 (필요할 때 펼치기)</strong></summary>

## 교정 스타일 지침이 유일한 미적 기준입니다

`references/original-prompt/zh-CN.md`는 교정된 020 지침이며 이 프로젝트의 유일한 창작·미적 기준입니다. Skill은 원문을 요약하거나 확장하지 않으며 공통 색상 계획, 미적 동기, 제목, 마이크로카피를 추가하지 않습니다. 색, 재료, 구성, 여백, 문구, 타이포그래피는 GPT Image 2가 원본 프롬프트의 규칙대로 수행합니다.
...
**물감에는 무게가 있고, 미니어처에는 사진의 기억이 남습니다.**
