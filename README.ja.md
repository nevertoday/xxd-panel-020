...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#4つの出力を支えるひとつの抽象記憶ロジック)
...
その違和感から **Panel 020** を作りました。テンプレートを被せるのではなく、写真から失ってはいけないものを見つけ、**写真を、極簡の抽象記憶と完全な編集システムへ**へ翻訳します。
...
<table>
  ...
  <tr>
    <td colspan="2"><img src="./assets/examples/sample-05.png" alt="XXD Panel 020 作例 5"></td>
  </tr>
  <tr>
    <td width="33%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 020 作例 6"></td>
    <td width="33%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 020 作例 7"></td>
    <td width="33%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 020 作例 8"></td>
  </tr>
</table>
...
これらの作例は 020 の美的意図を示すものであり、作例の被写体、構図、配色、コピー、旧キャンバス比率が生成時の参照や現在の既定値になることはありません。

> 分解—選択—凝縮—再構成 · 少数の形 · 厳密な色役割 · 象牙色の余白 · アートブックの小文字

XXD Panel 020 は、Codex と互換 Agent のための画像生成 Skill です。写真を分解、選択、凝縮、再構成し、最も固有な量感比、軸、曲線、交差、重層、遮蔽、負の空間、尺度差、非対称だけを少数の幾何形へ残し、一目で元写真に対応する抽象記憶を作ります。

色は元写真だけから取り、主色一つ、濃い構造色一つ、明るい／中性色一つ、必要なら小さな強調色へ限定します。2〜5語の詩的な題が感情を担い、2〜4組の小文字が秩序、根拠、素材、読解リズムを担います。幾何が視覚記憶を、文字が編集経路を作ります。
...
| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、極簡の抽象記憶と完全な編集システムへ |
| **ひと目で分かる特徴** | 分解—選択—凝縮—再構成 · 少数の形 · 厳密な色役割 · 象牙色の余白 · アートブックの小文字 |
...
一般的な「極簡幾何」は、汎用アイコン、任意の円と棒、均等配置、既成バウハウステンプレート、どの写真にも合う装飾形へ崩れがちです。

020 は順序を逆にします。

```text
量感／軸／曲線／交差／遮蔽／負空間／非対称を固定 → 構造事実を分解 → 認識に必要な少数関係を選択 → 明確な幾何役割へ凝縮 → 強い尺度階層で一つの重心へ再構成 → 主色／濃い構造色／明中性色を割り当て → 題と2〜4組の小文字で読解経路を作る
```

無関係な写真に替えても量感比、軸、曲線、交差、負形、色面積、文字経路がほぼ成立するなら、それは 020 ではありません。

## 完成作品で見分けやすいスタイル特性

- **元写真の構造：** 三つ以上の固有手掛かりが量感比、軸、曲線、交差、重層、遮蔽、負空間、尺度差、非対称から来ます。
- **四段階：** 構造事実を分解し、必要な関係を選び、幾何役割へ凝縮し、抽象記憶へ再構成します。
- **少数の形：** 元写真に根拠のある円、楕円、矩形、棒、円弧、楔、不規則面、簡潔な固有輪郭だけを使います。
- **一つの視覚記憶：** 強い尺度差、階層、正負形、遮蔽、広い象牙色の余白で一つの重心を作ります。
- **厳密な色役割：** 主色一つ、濃い構造色一つ、明るい／中性色一つ、必要なら小さな強調色だけです。
- **完全な文字系：** 2〜5語の題と2〜4組の小文字が、軸、端、輪郭、色面、負空間に沿って読解経路を作ります。
- **自動文案に年号なし：** 自動作成は年を発明・使用せず、ユーザーの確定文に年があれば一字一句保ちます。
...
**抽象は写真を消すことではなく、置き換えられない関係だけを残すこと。**
