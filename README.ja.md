...
| モード | サイズ方針 | 成果物 |
| --- | --- | --- |
| `top-bottom` | 元画像適応 | 上に完全な元写真、下に 020 の極簡抽象記憶編集。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `left-right` | 元画像適応 | 左に完全な元写真、右に 020 の極簡抽象記憶編集。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `design-only` | 元画像適応 | 元写真を見せず、変化デザインだけを表示。元画像の比率と寸法を維持 |
| `wallpaper-pack` | 端末別4サイズ | スマートフォン、iPad、デスクトップ、子ども用ウォッチの PNG を個別出力 |

優先順位は、正確な指定ピクセル > 指定比率／用途 > 通常モードの元画像適応です。原稿 `020.md` の 3:4 は初期の制作画角であり、現在の暗黙の既定値ではありません。
...
- **独立4作品：** 各端末は元写真だけを参照し、少数形体、正負空間、色面積、非対称重心、文字読解経路を別々に探れます。
...
自動文案は、光、空間、動き、素材、関係、感情から2〜5語の詩的な題を作り、2〜4組の小文字を加えます。題は感情を、小文字は秩序、根拠、素材、連続、細部を担います。

小文字には短句、状態語、確認済みの場所／対象情報、構成番号、章記号、連番、尺度風数字、方向語、素材語、記録ラベル、小説明を使えます。自動文案は年号を使わず、ユーザー確定文はそのまま保ちます。
...
画像モデルが元写真に結びついた少数の抽象形、量感と負空間の関係、厳密な色役割、完全な編集文字系を作ります。`scripts/compose_panel.py` は画布計画、厳密な 50/50 ラスター合成、最終寸法、監査だけを担当し、プログラム描画で作品を偽装しません。
...
- [中国語の完全プロンプト](references/xxd-panel-020-prompt.zh-CN.md)
- [英語の完全プロンプト](references/xxd-panel-020-prompt.en.md)
