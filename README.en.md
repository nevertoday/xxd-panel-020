...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 020 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 020's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 020 minimal abstract-memory editorial below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 020 minimal abstract-memory editorial right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `020.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
## Copy builds the reading path through abstract memory
...
Automatic copy distils one poetic 2–5-word title from light, space, action, material, relation, or emotion, then adds two to four microtext groups. The title carries emotion; the small text carries order, evidence, material, sequence, and detail.

Supporting groups may use a short phrase, state word, established place or object information, compositional index, chapter mark, sequence code, scale-like number, direction word, material word, archival label, or micro-explanation. Automatic copy uses no year; exact user wording stays verbatim. Copy must still pass the unrelated-image swap test.
...
The image model creates the source-bound few-form abstraction, mass and negative-space relationships, strict colour roles, and complete editorial type system. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-020-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-020-prompt.en.md)
- [Original style brief](references/020-source.md)
