...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#四种可组合输出模式)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 020 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 020 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 020 新增上下样张 3"></td>
...
**Panel 020** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**把照片嵌进真实厚涂，重构成可触摸的立体微景观**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
> 用 XXD Panel 020 帮我处理这张图，先推荐最合适的构图和尺寸。
...
[打开统一的多语言目录](references/original-prompt/)： [简体中文校正版](references/original-prompt/zh-CN.md) · [English](references/original-prompt/en.md) · [日本語](references/original-prompt/ja.md) · [한국어](references/original-prompt/ko.md) · [العربية](references/original-prompt/ar.md)

020 推文正文误重复了 015 的极简几何提示词，与“厚涂 × 立体感”的标题和四张原始样张冲突。[来源说明](references/original-prompt/source-tweet-body-2026-08-19.md)保留这一事实；简体中文校正版依据四张原始样张重建，并作为运行时唯一审美权威，其他四个版本是忠实阅读译文。

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 020 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 把照片重构成嵌入厚涂颜料的立体微缩场景 |
| **一眼可辨的特点** | 厚涂颜料岛 · 立体微缩场景 · 真实刀痕 · 大面积纸面留白 · 克制编辑排版 |
...
普通“厚涂风”很容易退化成一块装饰性泼墨、塑料 3D 玩具，或把完整场景缩小后贴进矩形画框；看似有质感，却与照片真正的主体关系无关。

020 的顺序完全相反：

```text
识别主体／动作／关系／情绪 → 删除无关背景 → 提炼源图最有力量的颜色关系 → 决定厚涂颜料岛的方向与尺度 → 将主体塑造成嵌入颜料的微缩实体 → 用刀痕、堆积、断边与投影建立真实触感 → 在纸面留白中加入极少量高度绑定的文字
```

如果换成一张无关照片，颜料岛、微缩主体、色彩关系和文案仍然成立，这张图就不属于 020。

## 成品中最容易识别的风格特点

- **源图身份：** 主体、动作和关键关系保持一眼可辨，不能被通用微缩景观替换。
- **厚涂颜料岛：** 一块有方向的斜向色带、刮擦色块或不规则颜料地形承担唯一视觉锚点。
- **真实材料：** 可见油画刀堆积、脊线、断裂边缘、露底、覆盖、高光和由厚度形成的柔和投影。
- **嵌入关系：** 微缩主体从颜料中生长，与其承托、嵌入、覆盖、堆叠、陷落或延伸，而不是贴在矩形框里。
- **源图配色：** 从照片最有辨识度和情绪力量的颜色关系中提炼主导厚涂色域，不使用固定色卡。
- **大面积留白：** 暖白、象牙白或自然纸白粗纹理纸面包围唯一场景，保持重量与呼吸感。
- **克制文字：** 模型文案必须与照片内涵高度绑定；用户准确文案逐字保留，文字轻放在留白中，不压过主体。
...
`references/original-prompt/zh-CN.md` 是本项目唯一的创作与审美权威。它依据 020 的四张原始 X 样张校正，避免继续误用与 015 重复的推文正文。Skill 不再额外总结或扩写它，也不会统一规划颜色、色板、美学动机、标题或微文案；其中怎样处理颜色、材料、构图、留白与文字，图像模型就按那套逻辑执行。
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 020 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-020 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 模型依照原始提示词决定文字，只限定语言
/xxd-panel-020 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-020 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-020 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 020；公共设置只解析一次
/xxd-panel-020 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
- [Skill 工作流](SKILL.md)
- [中文运行适配器](references/xxd-panel-020-prompt.zh-CN.md)
- [英文运行适配器](references/xxd-panel-020-prompt.en.md)
